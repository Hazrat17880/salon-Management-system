...
export default function NewPassword() {
  ...
  useEffect(() => {
    ...
    if (forgotData) {
      console.log("📦 forgotData from localStorage:", forgotData);
      setEmail(forgotData.email);
      setOtp(forgotData.otp); // ✅ if you saved otp in localStorage
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/auth/user/reset-password", {
        ...
        body: JSON.stringify({
          email,
          otp, // ✅ include OTP if required by backend
      ...
      >
        {!isSuccess ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-6">Your password has been successfully updated. You can now login with your new password.</p>
            <Link href="/login" className="inline-block px-6 py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition">
              Back to Login
            </Link>
          </div>
        )}
