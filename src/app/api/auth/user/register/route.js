...
export async function GET(request) {
  try {
    ...
    const sanitizedUsers = users.map(({ password_hash, otp_code, otp_expires_at, ...rest }) => rest);
...
export async function POST(request) {
  try {
    ...
    const otp_expires_at = new Date(Date.now() + 15 * 60 * 1000); // 15 min expiry
    ...
    // Get newly created user (no sensitive fields)
